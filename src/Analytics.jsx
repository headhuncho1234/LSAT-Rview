  ...
  return (
    <div style={{ marginBottom: 18 }}>
      ...
      <div className="perf-bar-wrap">
        <div className="perf-bar" style={{ width: `${pct}%`, background: pctColor(pct) }} />
      </div>
...
export default function Analytics({ history, xp, bestStreak, level, setScreen, setSection, setQType, generateQuestion }) {
  ...
  const typeArr = Object.values(typeMap)
    .filter(t => t.total >= 2)
    .sort((a, b) => (a.correct / a.total) - (b.correct / b.total))

  const handleDrill = (sec, type) => {
    setSection(sec)
    setQType(type)
    generateQuestion(sec, type)
  }
    ...
    return (
      <div className="screen" style={{ textAlign: 'center', padding: '60px 20px' }}>
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: 'var(--text-2)', fontSize: 13, marginBottom: 40, padding: 0 }}
        >
        ...
        <p style={{ color: 'var(--text-2)', fontSize: 14, marginBottom: 24 }}>
          Answer at least 5 questions to see your analytics.
        </p>
        <button className="btn btn-primary" style={{ margin: '0 auto' }} onClick={() => setScreen('setup')}>
          Start practicing
        </button>
      </div>
    )
  ...
  return (
    <div className="screen">
      <button
        onClick={() => setScreen('home')}
        style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: 'var(--text-2)', fontSize: 13, marginBottom: 24, padding: 0 }}
      >
        <ArrowLeft size={14} /> Back
      </button>
      ...
      <p style={{ fontSize: 14, color: 'var(--text-2)', marginBottom: 24 }}>
        Performance snapshot across {total} questions.
      </p>
      ...
      <div className="stat-grid" style={{ marginBottom: 24 }}>
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: pctColor(acc) }}>{acc}%</div>
          <div className="stat-sub">{correct}/{total} correct</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Est. score</div>
          <div className="stat-value" style={{ color: 'var(--purple)' }}>{scoreEst()}</div>
          <div className="stat-sub">target: 176–180</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Avg time</div>
          <div className="stat-value" style={{ color: avgTime <= 90 ? 'var(--green)' : 'var(--gold)' }}>{avgTime}s</div>
          <div className="stat-sub">{avgTime <= 90 ? 'on pace' : 'over target'}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Best streak</div>
          <div className="stat-value" style={{ color: bestStreak >= 5 ? 'var(--gold)' : 'var(--text)' }}>{bestStreak}</div>
          <div className="stat-sub">recent: {recentAcc}% (last 20)</div>
        </div>
      </div>

      {/* Per section */}
      <div className="card" style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 16 }}>
          By Section
        </div>
      ...
      {typeArr.length > 0 && (
        <div className="card" style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
              By Question Type
            </div>
            <span style={{ fontSize: 11, color: 'var(--text-3)' }}>sorted worst → best</span>
          </div>
      ...
      {/* 176+ target checklist */}
      <div className="card" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <Trophy size={16} color="var(--gold)" />
          <span style={{ fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-2)' }}>
            176+ Target Checklist
          </span>
        </div>
        {[
          { label: 'Overall accuracy ≥ 90%',      ok: acc >= 90,    note: `you're at ${acc}%` },
          ...
          <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, fontSize: 13.5 }}>
            <div style={{
              ...
              background: ok ? 'var(--green)' : 'var(--bg-elevated)',
              border: `1px solid ${ok ? 'var(--green)' : 'var(--border-md)'}`,
