    ...
    >
      ...
      {pct !== null ? (
        ...
        <div style={{ fontSize: 13, color: 'var(--text-3)', marginTop: 8 }}>
          No questions yet — click to start
        </div>
      )}
    ...
    <div className="screen">
      ...
      <div className="stat-grid" style={{ marginBottom: 24 }}>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: total >= 5 ? AccuracyColor(acc) : 'var(--text)' }}>{total > 0 ? `${acc}%` : '—'}</div>
      ...
      {total >= 10 && (
        <div className="card" style={{ marginTop: 24 }}>
          ...
          {[
            { label: 'Overall accuracy ≥ 90%', ok: acc >= 90 },
            { label: 'LR accuracy ≥ 88%', ok: (() => { const s = history.filter(h => h.section === 'LR'); return s.length >= 5 && (s.filter(h => h.correct).length / s.length) >= 0.88 })() },
            { label: 'RC accuracy ≥ 85%', ok: (() => { const s = history.filter(h => h.section === 'RC'); return s.length >= 5 && (s.filter(h => h.correct).length / s.length) >= 0.85 })() },
            { label: 'Avg LR time ≤ 90s', ok: (() => { const s = history.filter(h => h.section === 'LR'); return s.length >= 5 && s.reduce((a, h) => a + h.timeTaken, 0) / s.length <= 90 })() },
            { label: 'Best streak ≥ 10', ok: bestStreak >= 10 },
          ].map(({ label, ok }) => (
